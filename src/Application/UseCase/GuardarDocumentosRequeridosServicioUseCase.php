...
final class GuardarDocumentosRequeridosServicioUseCase
{
    /** @var list<string> */
     ...
     */
    public function __invoke(GuardarDocumentosRequeridosServicioInput $input): array
    {
        $servicioId = new ServicioId($input->servicioId);
        if (null === $this->servicioRepository->findById($servicioId)) {
            throw new \InvalidArgumentException('Servicio no encontrado.');
        }

        $documentos = $this->validateDocumentos($servicioId, $input->documentos);
        $this->repository->replaceForServicio($servicioId, $documentos);
     ...
     */
    private function serializeDocumento(ServicioDocumentoRequerido $doc): array
    {
        return [
            'id' => $doc->id()->value(),
            'fase' => $doc->fase()->value,
     ...
     */
    private function validateDocumentos(ServicioId $servicioId, array $raw): array
    {
        $documentos = [];
        ...
        foreach ($raw as $index => $item) {
            ...
            $id = is_string($idRaw) && '' !== trim($idRaw)
                ? new ServicioDocumentoRequeridoId($idRaw)
                : ServicioDocumentoRequeridoId::generate();

            $documentos[] = new ServicioDocumentoRequerido(
                $id,
                $servicioId,
                $fase,
                $nombre,
